...
Description:
    This program will reclassify a LAS/LAZ file point cloud based on a
    vegetation index selected from the following list:
        Excess Red (exr)
        Excess Green (exg)
        ...
        Combined Vegetation Index (com) *
             -  - > This index is a combination of (1) exg,
                                                   (2) exgr,
                                                   (3) cive, and
                                                   (4) vega

     * denotes vegetation indicies that appear to be unstable in simulated
     values (i.e. their values are not properly constrained).
...
Inputs:
    The program will automatically request the user to select 3 input files:
        ...
        2) The point cloud containing vegetation points only (for training).
            This input point cloud contains only points representative of
            vegetation and can be segmented out of the larger point cloud using
            the Segment tool in CloudCompare. Once the point cloud has been
            segmented, export the resuling sample as a LAS or LAZ file.
        3) The point cloud containing only bare Earth points (for training).
            This input point cloud contains only points representative of bare
            Earth and can be segmented out of the larger point cloud using the
            Segment tool in CloudCompare. Once the point cloud has been
            segmented, export the resuling sample as a LAS or LAZ file.

Outputs:
    A new LAZ file will be generated with the following naming scheme:
        {filename}_reclass_{maxidx}_veg_noveg.laz
    where {filename} is the original point cloud file name and {maxidx} is the
    name of the vegetation index selected as the most appropriate to
    differentiate vegetation from bare Earth using Otsu's thresholding
    approach. The output LAZ file will be saved in the same directory as the
    input file and will contain all the original points with updated
    classification values corresponding to vegetation or bare Earth.
...
# OPTIONAL INPUTS
# If you wanted to manually specify which vegetation index to use,
# then set defaults.veg_index equal to that abbreviation
defaults.veg_index = ''
...
def getfile(window_title='Select File'):
    ...
    root_win.withdraw()
    root_win.update()
    file_io = askopenfile(title=window_title)  # get user input for output dir
    root_win.destroy()  # destroy the tKinter window
    if not os.name=='nt':  # if OS not Windows, then correct slashes in dir name
...
def getdir(window_title='Select Directory'):
    ...
    root_win.withdraw()
    root_win.update()
    dir_io = askdirectory(title=window_title)  # get user input for output dir
    root_win.destroy()  # destroy the tKinter window
    if not os.name=='nt':  # if OS not Windows, then correct slashes in dir name
...
def getpcs():
    ...
    # initialize a list of acceptable input point cloud formats
    dc_formats = ["LAZ","laz","LAS","las"]
    # initialize empty list of dense clouds in the directory
    dc_in_dir = []
    # iterate through files in the input directory
    for file in os.listdir(in_dir):
        # exclude directories by checking that the "file" is not actually a dir
        if not os.path.isdir(ntpath.join(in_dir,file)):
            # if file extension is in list of acceptable point cloud formats
            if file.split(".")[1] in dc_formats:
                # IMPORTANT: CloudCompare requires standardized path slashes
                # all forward or backslashes MUST be OS standardized
                ...
                if os.name=='nt':  # check if the operating system is Windows
                    try:
                        ...
                        # MAY NOT WORK depending on how the command line is
                        # configured in Windows
                        dc_in_dir.append(os.path.join(in_dir,file))
                else:
                    # if NOT Windows, then the slashes should be properly
                    # configured with ntpath.join()
...
'''
Normalize values in the input array with the specified min and max values to the
output range normrange[].
'''
def normdat(inarr, minval=0, maxval=65536, normrange=[0,1]):
    # if inarr.dtype != np.float32:
    if not isinstance(inarr.dtype, np.float32):
...
def getminmax(rbands,gbands,bbands):
    ...
    if ((rmx>256) and (rmx<=65536)) or
       ((gmx>256) and (gmx<=65536)) or
       ((bmx>256) and (bmx<=65536)):
        rmx,gmx,bmx = 65536,65536,65536
    elif ((rmx>0) and (rmx<=256)) or
         ((gmx>0) and (gmx<=256)) or
         ((bmx>0) and (bmx<=256)):
...
The source for the code block is:
    https://stackoverflow.com/questions/48213278/implementing-otsu-binarization-from-scratch-python
     NOTE: The original answer does not include intensity. However, the
     follow-up answer (see the second code block) DOES include intensity in the
     computation of a threshold. Since we are trying to adapt this approach to
     only work with vegetation indicies, we do not need to include the recorded
     intensity when thresholding.
"""

'''
PURPOSE:
    Function to read in an array (combined from two training class histograms)
    and find a threshold value that maximizes the interclass variability.
...
def otsu_getthresh(gray, rmin, rmax, nbins):
    ...
    his,bins = np.histogram(gray,
                   np.arange(rmin.astype(np.float32),
                             rmax.astype(np.float32),
                             (rmax.astype(np.float32)
                              - rmin.astype(np.float32)) / nbins))
    final_thresh =  - 99999
    final_value =  - 99999
...
'''
PURPOSE:
    Function to apply a previously extracted threshold value to a new
    numpy array.

RETURN:
    Returns a copy of the input point cloud reclassed using the provided
    threshold and updated class values. (i.e. numpy array)
'''
...
'''
PURPOSE:
    Combine two input arrays without changing their values. This function
    utilizes pandas to combine the arrays without changing any array values.
...
NOTES:
    Using pandas DataFrames improves performance over solely using numpy arrays:
        mergehist() with Pandas concat() (i.e. first function):
            10 loops, best of 3: 19.8 ms per loop
        mergehist() with Numpy concatenate() (i.e. second function):
...
def mergehist(vegarr, novegarr, plothist=True):
    ...
    #     fig,ax = plt.subplots()
    #     ax.hist(df.values, bins=1000,
    #             weights=df_weights,
    #             color='black', histtype='step',
    #             label='vegarr', range=plt_range)
    #     ax.hist(df2.values, bins=1000,
    #             weights=df2_weights,
    #             color='green', histtype='step',
    #             label='novegarr', range=plt_range)
    #     ax.hist(df_merged.values, bins=1000,
    #             weights=df_merged_weights,
    #             color='red', histtype='step',
    #             label='Combined', range=plt_range)
...
'''
PURPOSE:
    Compute one or more vegetation indicies, as specified by the 'indicies'
    argument.

RETURN:
    Returns a pandas DataFrame object with the index (row names) as the
    vegetation index name and a 'values' column containing numpy arrays of the
    index values.
'''
def vegidx(r,g,b, indicies='all'):
    ...
    #     '''
    #     Visible Atmospherically Resistant Index (VARI)
    #        Gitelson, A.A.; Kaufman, Y.J.; Stark, R.; Rundquist, D. Novel
    #        algorithms for remote estimation of vegetation fraction. Remote Sens.
    #        Environ. 2002, 80, 76–87.
    #     '''
    #     vari = np.divide((g - r),
    #                      (g + r - b),
    ...
    #     '''
    #     Vegetativen (vega)
    #         Hague, T.; Tillett, N.D.; Wheeler, H. Automated crop and weed
    #         monitoring in widely spaced cereals. Precis.
    #         Agric. 2006, 7, 21–32.
    #     '''
    #     vega = np.divide(g,
    #                      (np.power(r,0.667) * np.power(b,(1 - 0.667))),
    #                      out=np.zeros_like(g),
    #                      where=(np.power(r,0.667) * np.power(b,(1 - 0.667))) != 0)
    #     pdindex.append([vega])
    #     pdindexnames.append('vega')
    # if indicies=='all' or indicies=='com':
    #     '''
    #     combined (COM) vegetation index from:
    #        Yang, W., Wang, S., Zhao, X., Zhang, J., and Feng, J. (2015).
    #        Greenness identification based on HSV decision tree. Information
    #        Processing in Agriculture 2, 149–160. doi:10.1016/j.inpa.2015.07.003.
    #     '''
    ...
    outdat = pd.DataFrame(pdindex,
                          index=pdindexnames,
                          columns=['vals'])
    outdat['minidxpos'] = minarr   # add new col w/ min possible index vals
    outdat['maxidxpos'] = maxarr   # add new col w/ max possible index vals
    return outdat
...
From:
    Kaufman, Y.J.; Remer, L.A. Detection of forests using mid - IR reflectance:
    An application for aerosol studies. IEEE Trans. Geosci. Remote Sens.
    1994, 32, 672–683.

The vegetation index with the greatest value, should provide the greatest
ability to differentiate the vegetation and non - vegetation classes.
'''
def computeM(intable1, intable2, writeout=''):
    outtable = pd.DataFrame(0.00,
                            index=list(intable1.index),
                            columns=['M - statistic'])
    for i in list(intable1.index):
        outtable['M statistic'][i] = (intable1['mean'][i] - intable2['mean'][i])
                                     / (intable1['std'][i] - intable2['std'][i])
    if writeout!='':
...
def main():
    ...
    classifyfile = 'C:/Users/pwernette/OneDrive  -  DOI/Machine Learning/test_dat/20180726.laz'      # for debugging on laptop
    infile_veg = 'C:/Users/pwernette/OneDrive  -  DOI/Machine Learning/test_dat/training_veg.laz'    # for debugging on laptop
    infile_noveg = 'C:/Users/pwernette/OneDrive  -  DOI/Machine Learning/test_dat/training_noveg.laz'   # for debugging on laptop

    # script timer
    startTime = time.time()

    '''
    Read in the complete LAS/LAZ file and the vegetation/no - vegetation
    LAS/LAZ Files
    '''
    ...
    print('Completed reading input LAS/LAZ files in {} seconds.'
    ...
    '''
    NOTE: Use deepcopy whenever copying variables, as it creates a stand - alone
    version of the variable and values, instead of simply referencing back to
    the original variable and values. Without deepcopy(), modifying a "copied
    object" may actually result in modifying the original object, instead of the
    copy.

    NEED a deepcopy() of each band for normalizing each band
    (see next code block)
    '''
    ...
    # vegetation training point clouds
    rv,gv,bv = deepcopy(fveg.red),deepcopy(fveg.green),deepcopy(fveg.blue)
    rvcopy,gvcopy,bvcopy = deepcopy(rv),deepcopy(gv),deepcopy(bv)

    # no vegetation training point clouds
    rn,gn,bn = deepcopy(fnoveg.red),deepcopy(fnoveg.green),deepcopy(fnoveg.blue)
    ...
    '''
    Extract the minimum and maximum values of the r,g,b bands for all combined
    point clouds (training and testing).

    In this case, since we are dealing with point clouds that have 16 - bit
    color depth, the minimum value possible is 0 and the maximum value possible
    is 65,535. If changing from 16 - bit color to 8 - bit color comment out the
    line with ranges 0 to 65,536 and uncomment the line with ranges 0 to 256.
    '''
    rmin,rmax,gmin,gmax,bmin,bmax = getminmax([rcopy,rvcopy,rncopy],
                                              [gcopy,gvcopy,gncopy],
                                              [bcopy,bvcopy,bncopy])

    '''
    Normalize the testing r,g,b values (then clean up the workspace)
    '''
    r = normRGB(normdat(rcopy,rmin,rmax),
                normdat(rcopy,rmin,rmax),
                normdat(gcopy,gmin,gmax),
                normdat(bcopy,bmin,bmax))
    g = normRGB(normdat(gcopy,gmin,gmax),
                normdat(rcopy,rmin,rmax),
                normdat(gcopy,gmin,gmax),
                normdat(bcopy,bmin,bmax))
    b = normRGB(normdat(bcopy,bmin,bmax),
                normdat(rcopy,rmin,rmax),
                normdat(gcopy,gmin,gmax),
                normdat(bcopy,bmin,bmax))
    ...
    '''
    rv = normRGB(normdat(rvcopy,rmin,rmax),
                 normdat(rvcopy,rmin,rmax),
                 normdat(gvcopy,gmin,gmax),
                 normdat(bvcopy,bmin,bmax))
    gv = normRGB(normdat(gvcopy,gmin,gmax),
                 normdat(rvcopy,rmin,rmax),
                 normdat(gvcopy,gmin,gmax),
                 normdat(bvcopy,bmin,bmax))
    bv = normRGB(normdat(bvcopy,bmin,bmax),
                 normdat(rvcopy,rmin,rmax),
                 normdat(gvcopy,gmin,gmax),
                 normdat(bvcopy,bmin,bmax))
    ...
    '''
    rn = normRGB(normdat(rncopy,rmin,rmax),
                 normdat(rncopy,rmin,rmax),
                 normdat(gncopy,gmin,gmax),
                 normdat(bncopy,bmin,bmax))
    gn = normRGB(normdat(gncopy,gmin,gmax),
                 normdat(rncopy,rmin,rmax),
                 normdat(gncopy,gmin,gmax),
                 normdat(bncopy,bmin,bmax))
    bn = normRGB(normdat(bncopy,bmin,bmax),
                 normdat(rncopy,rmin,rmax),
                 normdat(gncopy,gmin,gmax),
                 normdat(bncopy,bmin,bmax))
    ...
    '''
    Since the r, g, and b values have been normalized all based on the same
    minimum and maximim values
        AND
    Since none of the vegetation indicies use minimum or maximum of any band
        THEN...
    Computing the multiple (or individual) vegetation indicies yields the same
    range of values, regardless of whether it is a training sample or testing
    sample (again, assuming the first assumption holds true).
    '''
    # compute veg index from point cloud normalized and scaled rgb values
    vegidx_pts = vegidx(r,g,b)
    veg_idx = vegidx(rv,gv,bv)
    noveg_idx = vegidx(rn,gn,bn)

    veg_idx['label'] = 'vegetation'
    noveg_idx['label'] = 'no_vegetation'

    dat = pd.concat([veg_idx,noveg_idx])
    training = dat.sample(frac=0.7)
    split = int(len(filenames) * VALIDATION_SPLIT)

    training_filenames = filenames[split:]
    validation_filenames = filenames[:split]

    # compute summary stats (min, max, med, mean, and stdev) of each index
    # and append to the index table
    addstats(vegidx_pts)
    addstats(veg_idx)
    addstats(noveg_idx)
    ...
    '''
    The M statistic is an approach to identify how well normal distributions
    of the training classes, in this case vegetation and no vegetation, are
    distinguishable from each other. The greater the difference in
    distributions, the more differentiable they are from each other, and the
    greater the M statistic will be.
    ...
    Using the M statistic is taken from:
        ...
        Triviño-Tarradas, P., Jiménez-Brenes, F. M., García-Ferrer, A., et al.
        (2020). Classification of 3D Point Clouds Using Color Vegetation Indices
        for Precision Viticulture and Digitizing Applications. Remote Sensing
        12, 317. doi:10.3390/rs12020317.
    '''
    ...
    '''
    Get the index of maximum M - statistic (assuming it provides the greatest
    opportunity to differentiate vegetation points from non-vegetation points).

    Then, compute the minimum and maximum of the combined index values, which
    will be used to merge the two training classes/samples.
    '''
    ...
    minval = np.amin(np.concatenate((vegidx_pts['vals'][maxidx],
                                     veg_idx['vals'][maxidx],
                                     noveg_idx['vals'][maxidx])))
    maxval = np.amax(np.concatenate((vegidx_pts['vals'][maxidx],
                                     veg_idx['vals'][maxidx],
                                     noveg_idx['vals'][maxidx])))
    ...
    '''
    Compute the appropriate binarization threshold value for veg/no - veg
    samples using Otsu's thresholding method function otsu_getthresh().

    The 'nbins' value is simply used to determine how many bins will be used to
    divide the values into.
        More bins will INCREASE COMPUTATION TIME but will yield a MORE REFINED
        AND MOST APPROPRIATE binarization value.
        Fewer bins will DECREASE COMPUTATION TIME and yield a LESS REFINED
        binarization value.
    '''
    bin_thresh = otsu_getthresh(comboidx, minval, maxval,
                                nbins=defaults.thresholding_bins)

    '''
    Apply the threshold from the veg/no - veg binarization to another point
    cloud using the otsu_appthresh() function.
    '''
    updated_classes = otsu_appthresh(deepcopy(f.classification),
                                     vegidx_pts['vals'][maxidx],
                                     bin_thresh, reclasses=[2,4])

    '''
    Create copy of entire point cloud, update the classification field, and save
    the LAS/LAZ file.
    '''
    # create a new file ()"_reclass_veg_noveg.laz" appended to orig. filename)
    f_out = file.File(str(ntpath.join(root_dir,
                                      (infilename
                                           + '_reclass_'
                                           + str(maxidx)
                                           + '_veg_noveg.las')))
                                      .replace('\\','/'),
                                      mode='w', header=f.header)
    # copy the points from the origina file to the new output file
    f_out.points = f.points
    # update the classification with classes from the previous code block
    f_out.classification = updated_classes
    # close the output file
    f_out.close()
    '''
    CloudCompare cannot write out LAZ format natively. As a result, the output
    LAS files must be converted to LAZ compressed files using lastools command
    line utilities. Lastools must be installed and configured for command line
    usage before running this program.

    Although not required, the LAZ files require significantly less storage
    space than the original LAS output files.
    '''
