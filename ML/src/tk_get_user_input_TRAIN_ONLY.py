...
class App(tk.Tk):
    ...
    def create_widgets(self, default_arguments_obj):
        ...
        def browseFiles(intextbox, desc_text="Select File(s)"):
        ...
        lab.grid(column=0, row=rowplacement, sticky=W, padx=padxval, pady=padyval)
        model_name = Text(self, height=1, width=50)
        if default_arguments_obj.model_name == 'NA':
            model_name.insert(tk.END, 'model_'+default_arguments_obj.model_vegetation_indices+'_'+str(default_arguments_obj.model_nodes).replace(',','_').replace(' ','').replace('[','').replace(']',''))
        else:
            model_name.insert(tk.END, default_arguments_obj.model_name)
        model_name.grid(column=1, row=rowplacement, sticky=E, padx=padxval, pady=padyval)
        # increase the row by 1
        rowplacement += 1
        ...
        def getinput(self, default_arguments_obj):
            ...
            # model name
            if " " in model_name.get('1.0','end-1c'):
                default_arguments_obj.model_name = model_name.get('1.0','end-1c').replace(' ','_').split('\n')[0]
            ...
            if " " in model_nodes.get('1.0','end-1c'):
                ...
                default_arguments_obj.model_nodes = int(model_nodes.get('1.0','end-1c').split('\n')[0])
            # model dropout
...
class Args():
    ...
    def __init__(self, cname):
        ...
        self.gui = True
        # input files:
        #   vegetation: input LAS/LAZ files with training points separated by class
        # NOTE: Files should be split by class, where each input file corresponds
        # to a different type of class in the model and outputs.
        self.filesin = []
        # model name:
        self.model_name = 'NA'
    ...
    def parse_cmd_arguments(self):
        ...
        psr.add_argument('-m','-mname','-name','-modelname',
                         dest='modelname',
                         type=str,
                         default='NA',
        ...
        if args.modelnodes:
            ...
            if " " in args.modelnodes:
                self.model_nodes = list(map(int, str(args.modelnodes).split(',')))
            elif "," in args.modelnodes:
                self.model_nodes = list(map(int, str(args.modelnodes).split(',')))
            elif ";" in args.modelnodes:
                self.model_nodes = list(map(int, str(args.modelnodes).split(';')))
            else:
                self.model_nodes = int(str(args.modelnodes))
